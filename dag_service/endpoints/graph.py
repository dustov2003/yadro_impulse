...
@api_router.post(
    ...
    responses={
        400: {
            "model": ErrorResponse,
            "description": "Invalid graph data (e.g., duplicate nodes, cycle)",
        },
        422: {"model": HTTPValidationError, "description": "Validation Error"},
    },
)
async def create_graph(
    graph: GraphCreate, session: AsyncSession = Depends(get_session)
):
...
async def get_reverse_adjacency_list(
    graph_id: int = Path(..., title="Graph Id"),
    session: AsyncSession = Depends(get_session),
):

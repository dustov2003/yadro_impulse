...
class Node(DeclarativeBase):
    ...
    dag_id = Column(
        Integer, ForeignKey("dags.dag_id", ondelete="CASCADE"), nullable=False
    )
    name = Column(String(255), nullable=False)
    dag = relationship("DAG", back_populates="nodes")
    __table_args__ = (UniqueConstraint("dag_id", "name", name="unique_dag_node_name"),)
...
class Edge(DeclarativeBase):
    ...
    source_node_id = Column(
        Integer, ForeignKey("nodes.node_id", ondelete="CASCADE"), nullable=False
    )

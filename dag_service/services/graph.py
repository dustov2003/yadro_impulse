...
from dag_service.schemas import GraphCreate, GraphReadResponse, AdjacencyListResponse, ErrorResponse
...
async def get_graph_connections(dag_id: int, session: AsyncSession) -> Sequence[Row[Any]]:
...
async def read_graph(dag_id: int, session: AsyncSession) -> tuple[Sequence[Node], Sequence[Row[Any]]]:
...
async def delete_node(dag_id: int, node_name: str, session: AsyncSession):
    count_nodes = await count_nodes_by_dag_id(dag_id, session) # по сути так можно узнать одновременно существует ли такой граф (так как у графа минимум одна нода) и узнать количество нод
    if count_nodes < 1:
        raise HTTPException(status_code=404, detail=ErrorResponse(message='ladno').model_dump())

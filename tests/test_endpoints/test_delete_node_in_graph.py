...
class TestDeleteNodeInGraph:
    ...
    def test_base_scenario(self, client, dag_sample):
        ...
        assert response.json() == {
            ...
            "nodes": [{"name": "A"}, {"name": "B"}, {"name": "D"}, {"name": "E"}],
            "edges": [{"source": "A", "target": "B"}, {"source": "D", "target": "E"}],
        }

    def test_graph_not_found(self, client, dag_sample):
        response = client.delete(
            url=self.get_url(dag_sample.dag_id + 99, "A")
        )  # тот dag_id которого точно нет
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_node_not_found_in_graph(self, client, dag_sample):
        response = client.delete(
            url=self.get_url(dag_sample.dag_id, "NONEXISTENNODE")
        )  # тот dag_id которого точно нет
        assert response.status_code == status.HTTP_404_NOT_FOUND
